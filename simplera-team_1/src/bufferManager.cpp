...
/**
 * @brief Removes a page from the pool (RAM) without deleting the file (Disk).
 * Necessary when the file on disk has been updated externally (e.g., by sort).
 */
void BufferManager::evictFromPool(string tableName, int pageIndex)
{
    string pageName = "../data/temp/" + tableName + "_Page" + to_string(pageIndex);
    
    // Iterate and erase if found
